...
export function Card({ title, description, imageUrl, link, cta }: CardProps) {
  return (
    <div
      className="rounded-lg shadow-md overflow-hidden bg-background h-full flex flex-col relative group"
    >
      {link && (
        <Link
          href={link}
          className="absolute inset-0 z-10 focus:inset-2 focus:inline-primary rounded-lg"
          aria-label={title}
        />
      )}
      {imageUrl && (
